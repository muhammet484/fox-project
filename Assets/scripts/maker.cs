...
using System.Linq;
using UnityEngine;
// maker maker chicken shaker yeeeey
    ...
    void Update()
    {
        if ( player.transform.position.z > theObjectThatIsInTheLocatıonOfPlayer(isBehindIt))
        {
            ...
            GameObject _choosenOneObject = choosenObject();
                ...
                mAIN_TRANSPORTER_SCRIPT.allStuffs.Add(
                Instantiate(_choosenOneObject, new Vector3(0, 0, 0 + (endOfRoad + (lenght / 2))), Quaternion.identity)
                );
                endOfRoad += lenght;
                _new_object_count++;
            }
    ...
    GameObject choosenObject()
    {
        ...
        if (isFirst)
        {
            if (choosenObjectNumber < 10)
            {
                ...
                return ThemeManager.themeEngine[0].themeObjectsList[0];
            }
        }
        else
        {
 ...
 */
    ...
    float theObjectThatIsInTheLocatıonOfPlayer(bool _isbehindit)
    {
        if (_isbehindit)
        {
            return _theObjeThatIsInTheLocatıonOfPlayer[_theObjeThatIsInTheLocatıonOfPlayer.Count - 1].transform.GetComponent<BoxCollider>().size.z 
                + _theObjeThatIsInTheLocatıonOfPlayer[_theObjeThatIsInTheLocatıonOfPlayer.Count - 1].transform.position.z
                + extraDestination
                ;
        }
        else
        {
            return _theObjeThatIsInTheLocatıonOfPlayer[0].transform.GetComponent<BoxCollider>().size.z
